...
/**
 * Cmd/Ctrl+K search for the docs portal. Mirrors the main app's
 * `src/components/navigation/command-search.tsx` visual/animation
 * convention (same Portal + backdrop + centered panel technique) but reads
 * from the docs search index instead of the main app's nav items, and adds
 * a small "recent searches" list.
 */
...
import type { useDocsSearch } from "../hooks/use-docs-search";
...
/** Takes the caller's single `useDocsSearch()` instance as a prop rather than calling the hook itself — the header's search trigger and this dialog must share ONE state instance, or opening the dialog from the header would toggle a different `open` flag than the one this component renders from. */
export function DocsSearchDialog({ search }: { search: ReturnType<typeof useDocsSearch> }) {
                ...
                {query.trim() === "" ? (
                  recent.length > 0 ? (
                    ...
                    <p className="text-muted-foreground px-3 py-6 text-center text-sm">
                      Search for pages, API methods, hooks, and guides.
                    </p>
                  )
                ) : results.length === 0 ? (
                  <p className="text-muted-foreground px-3 py-6 text-center text-sm">
